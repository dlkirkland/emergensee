...
class MainHandler(webapp2.RequestHandler):
    ...
    def _create_note(self, user):
        #create "note" as an object instance of the class "Note".  Included here must be all parts of the class
        #example = User, title, content
        note = Note(parent=ndb.Key("User", user.nickname()),
                    title=self.request.get('title'),
                    content=self.request.get('content'))
        #object name "note" will upload the information to something
        note.put()
    ...
    #when the save button is pressed, self._create_note(user) is called and the note is saved
    def post(self):
        user = users.get_current_user()
        if user is None:
            self.error(401)
